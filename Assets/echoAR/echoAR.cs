...
public class echoAR : MonoBehaviour
{
    ...
    IEnumerator DownloadAssets(string serverURL,string reqModel)
    {
        ...
        foreach (Entry entry in dbObject.getEntries())
        {
            ...
            if (entry.getAdditionalData() != null && entry.getAdditionalData().TryGetValue("type", out value))
            {
                ...
                {
                    string Header = "";
                    if (entry.getAdditionalData().TryGetValue("name", out Header))
                    {
                        headerText.text = Header;
                    }

                    if (entry.getAdditionalData().TryGetValue("question", out Header))
                    {
                        questions = new List<string>(Header.Split(';'));

                    }
                    if (entry.getAdditionalData().TryGetValue("answer", out Header))
                    {
                        answers = new List<string>(Header.Split(';'));
                    }
                    ...
                    {
                        nextQues = false;
                        question.GetComponentInChildren<Text>().text = questions[i];
                        answer.GetComponentInChildren<Text>().text = answers[i];
                        Debug.Log("Gotcha");
                        i += 1;
                        yield return new WaitUntil(() => nextQues != false);
                    }
    ...
    public void GetType(string type)
    {
        ...
        try
        {
            StartCoroutine(QueryDatabase(serverURL,type));
        }
        catch (System.Exception e)
        {
            Debug.Log(e);
